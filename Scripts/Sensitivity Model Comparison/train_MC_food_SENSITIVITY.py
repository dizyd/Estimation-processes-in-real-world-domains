"""
BayesFlow: Personwise Model Comparison (Food) - Sensitivity Analysis
Converted from BF2_Personwise_MC_food.ipynb for cluster execution.
...
design_csv = os.path.join(MATERIALS_DIR, "design_data_food.csv")
data_csv   = os.path.join(DATA_DIR, "data_analysis_food.csv")
...
# 2.1 CAM
result = sm.ols(
    formula="crit ~ " + " + ".join(f"V{i}" for i in range(1, 15)),
    data=df,
).fit()
...
model_path = os.path.join(NETWORKS_DIR, f"sensitivity_MC_FOOD_{network}.keras")
...
csv_path = os.path.join(RESULTS_DIR, f"pmp_FOOD_{network}.csv")
