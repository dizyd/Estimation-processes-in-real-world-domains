"""
BayesFlow: Personwise Model Comparison (Countries) - Sensitivity Analysis
Converted from BF2_Personwise_MC_countries.ipynb for cluster execution.
...
design_csv = os.path.join(MATERIALS_DIR, "design_data_countries.csv")
data_csv   = os.path.join(DATA_DIR, "data_analysis_countries.csv")
...
# 2.1 CAM
result = sm.ols(
    formula="crit ~ " + " + ".join(f"V{i}" for i in range(1, 11)),
    data=df,
).fit()
...
# Replace NaNs with the row mean across the other participants (exclude the NaN
# itself and the participant that has the NaN from the mean).
meta_cols     = ["ID_item", "item", "img", "training", "crit"]
response_cols = [col for col in df_data.columns if col not in meta_cols]

meta_df   = df_data[meta_cols].copy()
responses = df_data[response_cols].copy()
...
model_path = os.path.join(NETWORKS_DIR, f"sensitivity_MC_COUNTRIES_{network}.keras")
...
# NOTE: the first predict() call JIT-compiles the inference graph (can take ~30-60s on
# GPU); the tqdm bar makes the otherwise-silent loop visibly progress afterwards.
...
csv_path = os.path.join(RESULTS_DIR, f"pmp_COUNTRIES_{network}.csv")
